...
GameSys = (function() {
  var GameSysPrivate, _instance, _scheduler;
  ...
  GameSys = function(w, h) {
    return _instance != null ? _instance : _instance = new GameSysPrivate(w, h);
  };

  GameSysPrivate = (function() {
    var $, FPS, _game, _vZIndex;
    ...
    GameSysPrivate.prototype.preload = function(objs) {
      return _game.preload(objs);
    };

    GameSysPrivate.prototype.start = function() {
      var d;
      d = new $.Deferred;
      _game.start();
    ...
    GameSysPrivate.prototype.getCurrentScene = function() {
      return _game.currentScene;
    };

    GameSysPrivate.prototype.getChildNum = function(obj) {
      return obj.childNodes.length;
    };

    GameSysPrivate.prototype.createScene = function() {
      var scene;
      return scene = new Scene();
    };

    GameSysPrivate.prototype.replaceScene = function(scene) {
      _game.replaceScene(scene);
      return _vZIndex.length = 0;
    };

    GameSysPrivate.prototype.createSprite = function(w, h, src) {
      var sprite;
    ...
    GameSysPrivate.prototype.setImage = function(obj, src) {
      return obj.image = _game.assets[src];
    };

    GameSysPrivate.prototype.setOpacity = function(obj, opacity) {
      return obj.opacity = opacity;
    };

    GameSysPrivate.prototype.getOpacity = function(obj) {
      return obj.opacity;
    };

    GameSysPrivate.prototype.createLabel = function(font, color, align) {
      var label;
      label = new Label();
      label.font = font;
      label.color = color;
    ...
    GameSysPrivate.prototype.creatAudio = function(src) {
      return _game.assets[src];
    };

    GameSysPrivate.prototype.playAudio = function(audio) {
      return audio.play(true);
    };

    GameSysPrivate.prototype.createGroup = function() {
      return new Group();
    };

    GameSysPrivate.prototype.createNode = function() {
      return new Node();
    };

    GameSysPrivate.prototype.setScheduler = function(func, obj) {
      var j, len, node, v;
      if (obj != null) {
        obj.addEventListener("enterframe", func);
        return obj;
      } else {
        for (j = 0, len = _scheduler.length; j < len; j++) {
          v = _scheduler[j];
          if (!v.active) {
            v.active = true;
        ...
        node = new Node();
        node.active = true;
        this.addChild(this.getCurrentScene(), node);
    ...
    GameSysPrivate.prototype.clearScheduler = function(obj) {
      obj.clearEventListener("enterframe");
      return obj.active = false;
    };

    GameSysPrivate.prototype.setText = function(label, text) {
      return label.text = text;
    };

    GameSysPrivate.prototype.addChild = function(dist, obj, z) {
      var i, j, len, v;
      obj.z = z;
      if (z != null) {
        for (i = j = 0, len = _vZIndex.length; j < len; i = ++j) {
          v = _vZIndex[i];
          if (!(v.z > z)) {
            continue;
          }
        ...
        return dist.addChild(obj);
      }
    };

    GameSysPrivate.prototype.removeChild = function(dist, obj) {
      var i, j, len, v;
      if (obj.z != null) {
        for (i = j = 0, len = _vZIndex.length; j < len; i = ++j) {
          v = _vZIndex[i];
    ...
    GameSysPrivate.prototype.poolObj = function(pool, obj) {
      pool.push(obj);
      obj.active = false;
      if (obj.addEventListener) {
        return obj.addEventListener("removed", this._deactive);
      }
    };

    GameSysPrivate.prototype.getObj = function(pool) {
      var i, j, len, v;
      for (i = j = 0, len = pool.length; j < len; i = ++j) {
        v = pool[i];
        if (!v.active) {
          v.active = true;
          pool.lastObj = i;
    ...
    GameSysPrivate.prototype.getLastActiveObj = function(pool) {
      return pool[pool.lastObj];
    };

    GameSysPrivate.prototype.setFrame = function(obj, num) {
      return obj.frame = num;
    };

    GameSysPrivate.prototype.allocateObjs = function(objs) {
      var k, obj, results, v;
      results = [];
      for (k in objs) {
        v = objs[k];
        if (v.type === "image") {
          obj = this.createSprite(v.width, v.height, v.src);
        } else if (v.type === "label") {
          obj = this.createLabel(v.font, v.color, v.align);
        }
        obj.x = v.x;
        obj.y = v.y;
        results.push(this.addChild(this.getCurrentScene(), obj, v.z));
      }
      return results;
    };

    GameSysPrivate.prototype.removeAllObjs = function(dist) {
      var j, len, ref, results, v;
      ref = dist.childNodes;
      results = [];
      for (j = 0, len = ref.length; j < len; j++) {
        v = ref[j];
        results.push(this.removeChild(dist(v)));
      }
      return results;
    };

    GameSysPrivate.prototype.deactive = function() {
      return this.active = false;
    };

    return GameSysPrivate;
...
generateKeyDownEvent = function(key) {
  var event;
